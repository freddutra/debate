...
function local_problemsection_create($data) {
    ...
    $data->name = "Debate Crítico - " . $data->title;
    $data->summary = array("text"=>"Sumario temporário", "format"=>1);
    ...
    //$problemsectionid = local_problemsection_record($data, $section, $groupingid, "");
    $problemsectionid = local_problemsection_record($data, $section, "");
    ...
    $quiz->name = "Quiz para Debate crítico: " . $data->title;
    $quiz->intro = "Acerca do tema (" . $data->title . "), selecione uma das opções abaixo:" ;
    $quiz->introformat = 1;
    $quiz->display = 0;
    ...
    $quizmod->section = 0;
    $quizmod->added = time();
    $quizmod->timeopen = time();
    $quizmod->timeclose = time() + (365 * 24 * 60 * 60); // One year from now
    ...
    //print_r($problemsectionid);

    //Armazena dados no DB
    try{
    $statusdata = new stdClass();
    $statusdata->courseid = $data->courseid;
    $statusdata->problemid = $problemsectionid;
    $statusdata->debateid = uniqid("", true);
    $statusdata->sessionid = "";
    $statusdata->debatename = $data->title;
    $statusdata->sessionid = $section->id;
    ...
    $statusdata->forumrefuteid = 0;
    $statusdata->sequence = "";
    $DB->insert_record('local_problemsection_status', $statusdata);
    echo "<pre>"; echo "Gravado com sucesso";
    }
 ...
 * @return type
 */
function local_problemsection_record($data, $section, $sequence) {
    global $DB;
